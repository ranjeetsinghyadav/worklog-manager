    ...
    <s2ui:form width='100%' height='225' elementId='formContainer'
        ...
        <g:form action='persistentLoginSearch' name='persistentLoginSearchForm'>
            ...
            <table>
                ...
                <tr>
                    <td colspan='2'><s2ui:submitButton elementId='search' form='persistentLoginSearchForm'
            ...
            <table>
                ...
                <tr>
                    <g:sortableColumn property="id"
                                      title="${message(code: 'persistentLogin.series.label', default: 'Series')}"
                                      params="${queryParams}"/>
                    <g:sortableColumn property="username"
                                      title="${message(code: 'persistentLogin.username.label', default: 'Username')}"
                                      params="${queryParams}"/>
                    <g:sortableColumn property="token"
                                      title="${message(code: 'persistentLogin.token.label', default: 'Token')}"
                                      params="${queryParams}"/>
                    <g:sortableColumn property="lastUsed"
                                      title="${message(code: 'persistentLogin.lastUsed.label', default: 'Last Used')}"
                                      params="${queryParams}"/>
                </tr>
                </thead>
                ...
                <g:each in="${results}" status="i" var="persistentLogin">
                    <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
                        <td><g:link action="edit"
                                    id="${persistentLogin.id}">${fieldValue(bean: persistentLogin, field: "id")}</g:link></td>
                        <td><g:link controller='user' action='edit'
                                    params='[username: persistentLogin.username]'>${fieldValue(bean: persistentLogin, field: "username")}</g:link></td>
